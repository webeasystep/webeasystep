    ...
    public function importQuiz()
    {
        if ($this->request->getMethod() === 'POST') {
            ...
            if (!$file || !$file->isValid()) {
                return redirect()->back()->with('error', lang('Quizzes.please_select_valid_json_file'));
            }

            if ($file->getClientExtension() !== 'json') {
                return redirect()->back()->with('error', lang('Quizzes.only_json_files_allowed'));
            }
            ...
            if ($this->quizzesModel->importFromJSON($quizData)) {
                return redirect()->to('/dt_admin/quizzes')->with('success', lang('Quizzes.quiz_imported_successfully'));
            } else {
