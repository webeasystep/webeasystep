...
class SMSActivator implements ActionInterface
{
    ...
    public function show(): string
    {
        ...
        $sms= object();
        // Send the email
        $sms->setFrom(setting('Email.fromEmail'), setting('Email.fromName') ?? '');
        $sms->setTo($userEmail);
        $sms->setMessage(view(setting('Auth.views')['action_email_activate_email'], ['code' => $code]));
        if ($sms->send(false) === false) {
